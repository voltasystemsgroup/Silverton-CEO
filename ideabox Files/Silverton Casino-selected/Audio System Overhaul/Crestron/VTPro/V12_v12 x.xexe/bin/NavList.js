...
*/
function NavList(Document, Properties)
{
    ...
    this.SelectionY = parseInt(((this.Height / 2) + this.y) - (this.ItemHeight / 2))
        
    this.ItemX = Properties["Item_Offset"] + this.x
    
    this.ControlHeight = Properties["Header_Height"]    
    
    this.WrapTop =  this.y + this.ControlHeight - this.ItemHeight
    
    if (((this.ItemHeight * (this.ItemCount - 1)) + (this.ItemSpacing * (this.ItemCount - 1))) >= this.Height)
    {
        // maybe we should just expand the spacing? at least then it would scroll
        DebugPrint("NavList: invalid sizes...\n")
        return null
    }
    ...
    this.UpButton = this.Page.GetObject(Properties["#Name"] + "|Up")
    ...
    this.UpButton.Height = this.ControlHeight
        
    this.DownButton = this.Page.GetObject(Properties["#Name"] + "|Down")
	if (Properties["Suppress Keyclicks"] != "Off")
    ...
    this.DownButton.y = this.y + this.Height - this.ControlHeight
    this.DownButton.Width = this.Width
    this.DownButton.Height = this.ControlHeight
    ...
    */
    ...
    for (i = 0; i < this.ItemCount; i++)
    {
        ...
        hdr.y = this.UpButton.y + this.UpButton.Height + (hdr.Height * i) + (this.ItemSpacing * i)

		if (hdr.y > this.DownButton.y)
		{
			hdr.Visible = false;
		}
...
NavList.prototype.HasValidSelection = function()
{
    return this.IsValidSelection(this.SelectedIndex)       
}
NavList.prototype.IsValidSelection = function(item_index)
{
...
NavList.prototype.SetSelected = function(item_index)
{
    var itm_x
...
NavList.prototype.StartAnimation = function()
{    
    try
    {
...
NavList.prototype.StopAnimation = function()
{
...
NavList.prototype.SlideTo = function(item_index)
{
    ...
    for (var i = 0; i < this.ItemCount; i++)
    {
        ...
        if (itm_x.ptr.y < lowest_y)
        {
            lowest_index = itm_x.SelectionIndex
            lowest_y = itm_x.ptr.y
        }
            
        if (itm_x.ptr.y > highest_y)
        {
            highest_index = itm_x.SelectionIndex
            highest_y = itm_x.ptr.y
        }
   }
    ...
    {
        this.Dir = 1
        this.SlideAmount = low_diff * (this.ItemHeight + this.ItemSpacing) + (this.SelectionY - lowest_y)
    }
    else
    {
        this.Dir = -1
        this.SlideAmount = high_diff * (this.ItemHeight + this.ItemSpacing) + (highest_y - this.SelectionY)
    }
...
NavList.prototype.OnSendDigital = function(item, x, y, info)
{
	try
	{
...
NavList.prototype.OnPressItem = function(item, x, y, info)
{
...
NavList.prototype.OnPressMove = function(item, x, y, dir)
{
    item.State = 1
	item.Refresh()
...
NavList.prototype.OnReleaseMove = function(item, x, y)
{
    this.StopAnimation()        
...
NavList.prototype.GetItem = function(item_index)
{
    for (i = 0; i < this.ItemCount; i++) 
    {
...
NavList.prototype.SnapbackCheck = function(tmr)
{
    this.SnapbackTimer.Stop()
    ...
    if (selected_item != null)
        if (selected_item.y == this.SelectionY)
            return
    
    this.SlideTo(this.SelectedIndex)
}
NavList.prototype.BrowseTimer = function(tmr)
{
    if (this.last_tick <= 0)
    ...
    for (i = 0; i < this.ItemCount; i++) 
    {
        ...
        if (this.Dir > 0)
        { // we're animating up
            if (tmp_item.y > this.DownButton.y)            
            {
                ...
                newy = prev_info.ptr.y - this.ItemHeight - this.ItemSpacing
		...
		if (tmp_item.y < this.UpButton.y || tmp_item.y + tmp_item.Height > this.DownButton.y + this.DownButton.Height)
...
NavList.prototype.OnDigital = function(conn, join, value)
{    
...
NavList.prototype.OnAllClear = function(conn)
{

}
NavList.prototype.OnNetCipConnect = function(conn)
{	
